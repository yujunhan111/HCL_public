"""
Downstream linear regression task with variable sample size (m).

This module evaluates the performance of learned representations on a downstream
linear regression task by varying the sample size of the training data (m) while
keeping the representation learning data size fixed (n=60).
...
def run_one_rep(ii, m, c, d1, d2, d3, r, dvc):
    """
    Run one repetition of downstream regression experiment with variable sample size.
    
    Procedure:
    1. Generate representation learning data (fixed n=6*10)
    2. Estimate noise variance and clean covariance
    3. Learn weight matrix using HCL-SVD and gradient refinement
    4. On downstream task with varying m: fit OLS and Group Lasso
    5. Evaluate using L2 distances and excess risk
    
    Args:
        ii: int, repetition index
        m: int, sample size for downstream task
    ...
    d = d1+d2+d3
    # Representation learning data (large sample)
    x1, W = Data_generate(d1, d2, d3, r=r, n=10*6, c=c, device=dvc)
    ...
    # Learn weight matrix: HCL-SVD + gradient refinement
    ...
    # Method 2: Group Lasso estimator
...
if __name__ == '__main__':
    a = run_one_rep(0, 5000, 10, 100, 500, 800, 10, dvc="cuda")
